...
for i in range(Xs_t_test.shape[0]):
    if i == 0:
        ...
        label = None
    x = Xs_t_test[i]
    plt.plot(tm, x[:H, :dP], ls='--', color='k', alpha=0.4, label=label)
plt.legend()

plt.subplot(122)
for i in range(Xs_t_test.shape[0]):
    x = Xs_t_test[i]
    plt.plot(tm, x[:H, dP:dP+dV], ls='--', color='k', alpha=0.4)
...
plt.show(block=False)
print('NLL-40:')
print('GP:', blocks_gp_bd_results['nll'])
...
print('BNN:', np.mean(blocks_bnn_bd_results['rmse']))
print('MOE:', blocks_moe_bd_results['rmse'])

print('Pred-time-40:')
print('GP:', blocks_gp_bd_results['gp_pred_time'])
print('BNN:', np.mean(blocks_bnn_bd_results['pred_time']))
print('MOE:', blocks_moe_bd_results['moe_pred_time'])

print('Train-time-40:')
print('GP:', blocks_gp_bd_results['gp_training_time'])
print('BNN:', np.mean(blocks_bnn_bd_results['train_time']))
