...
from mixture_gp_model_gibbs_sampling import *
...
discon_params = {
            ...
            'Nsam': 1,
            'Nsec': 2, # number of clusters in the dataset
            'noise_gain': 2.,
            # 'noise_gain': 0,
            'disc_flag': True,
            'lin_m': -7.,
...
init_state = {
            ...
            'num_clusters_': 2, # K, use K=1 for sequencial initial assignment for DP
            ...
            'hyperparameters_': {
                                ...
                                'gp_params': {'signal_variance': None, # to be init with empirical
                                              'K_RBF': RBF(1, (1e-3, 1e3)),
                                              'noise_variance': 2., # should be the same as noise level below
                                              'K_W': W(noise_level=2., noise_level_bounds=(1e-1, 1e1)),
                                              'normalize_y': True,
                                              'restarts': 0,
                                              }
                                },
...
for chain_num in range(init_count):
    ...
    chain_stat = run_Markov_chain_gp(init_state, data, DP_flag,rand_clust_size,seq_init,max_itr)
...
pickle.dump( exp_result, open( "syn_1d_infinite_gp_clustered.p", "wb" ) )
